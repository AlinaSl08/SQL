...
`RIGHT JOIN` возвращает все строки из правой таблицы, добавляет соответствующие строки из левой таблицы, заполняет NULL столбцы левой таблицы, если нет совпадений.
...
**Запрос:**
``` SQL
SELECT u.name, o.id AS order_id, o.total_price
...
|:--:|:--:|:--:|
|Иван Петров	|101|	2990|
|Елена Орлова|	102|	4500|
|NULL	103|	8000|
...
**`NULL`** – это специальное значение, обозначающие отсутствие значения или неизвестное значение. Это не пустая строка (''), не ноль (0) и не строка из пробелов.

**Важно**: любое сравнение с `NULL` через стандартные операторы (=, <>, >, < и др.) не работает. Поэтому для работы с `NULL` нужны специальные операторы.


**Основные операторы для работы с `NULL`:**
...
3. В одном запросе можно использовать одновременно `IS` и `IS NOT`
...
WHERE колонка IS NULL AND колонка IS NOT NULL;
```
...
**Ошибка:**
``` SQL
WHERE колонка = NULL -- НЕ РАБОТАЕТ!
```
                  
**Почему?**

+ В SQL любое сравнение с `NULL` даёт результат `UNKNOWN`.

+ Условие `WHERE` принимает только строки с результатом `TRUE`.

+ Поэтому запрос с `= NULL` **никогда** не вернёт строк, даже если в таблице есть `NULL` значения.

 

**Пример 1: найти заказы без даты доставки**

**таблица `orders`**

|id	|user_id|	total_price|	delivery_date|
|:--:|:--:|:--:|:--:|
|1	|101|	2990	|2025-01-10|
|2|	102|	4500	|NULL|
|3|	103	|1500|	2025-01-12|
|4	|104	|3200|	NULL|
...
**Пример 2: найти заказы, где дата доставки уже указана**
...
`DISTINCT` – это оператор в SQL, который исключает дублирующиеся строки из результата запроса. Он оставляет только уникальные (неповторяющиеся) значения в выбранных столбцах.

**Синтаксис**
``` SQL
SELECT DISTINCT колонка
...
```
 ...
 В этом случае уникальность считается по паре (`колонка1` + `колонка2`).
...
``` SQL
1. Показать список уникальных статусов заказа: SELECT DISTINCT status FROM orders;
```
...
``` SQL
2.  Запрос DISTINCT по нескольким полям: SELECT DISTINCT user_id, status FROM orders;
```
